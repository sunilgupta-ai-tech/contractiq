...
POST /query orchestration: validation, conversation, pipeline, persistence.
    ...
    4. answer           mode "agent" (default): LangGraph agent — rewrite,
                        decompose, retrieve, validate, refine, answer, cite;
                        mode "fast": single-pass QAPipeline (Phase 7)
...
def _agent_runner() -> type[AgentRunner]:
    """Load the LangGraph agent on first use, not at API startup.

    Importing LangGraph only when agent mode is used keeps API start-up fast,
    and means a missing agent dependency makes agent-mode questions fail with
    a clear "not configured" 503 instead of preventing the whole API (auth,
    uploads, health checks) from starting.
    """
    try:
        from app.agents.graph import AgentRunner
